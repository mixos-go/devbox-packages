...
class RuntimeConfig(BaseModel):
    """Runtime selection (docker, kubernetes, proot, etc.)."""

    # BUG 7 FIX: "proot" was missing from the Literal. DevBox config.toml sets
    # runtime.type = "proot" which caused a Pydantic ValidationError on startup.
    type: Literal["docker", "kubernetes", "proot"] = Field(
        ...,
        description="Active sandbox runtime implementation.",
    )
    execd_image: Optional[str] = Field(
        default=None,
        description="Container image that contains the execd binary for sandbox initialization. "
                    "Not required when runtime.type = 'proot'.",
        min_length=1,
    )
...
class AppConfig(BaseModel):
    ...
    def validate_runtime_blocks(self) -> "AppConfig":
        ...
        elif self.runtime.type == "proot":
            # BUG 7 FIX: proot mode is lightweight — no kubernetes/docker/execd_image required.
            # Silently ignore kubernetes/docker blocks if present (install.sh may not strip them).
            pass
        else:
