...
# BUG 8 FIX: import ProotSandboxService statically so it is always available.
# Previously it was only added by install.sh's sed patch, which could silently
# fail and leave factory.py without the crosvm entry — crashing the server with
# ValueError("Unsupported sandbox service type: crosvm") on every request.
from src.services.crosvm_service import CrosvmSandboxService
