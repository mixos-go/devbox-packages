# DevBox crosvm runtime for OpenSandbox
# Implements SandboxService using local processes inside the Debian VM (via crosvm).
...
# BUG 7 - RuntimeConfig.type Literal missing "crosvm" → fixed in config.py
...
class _SandboxRecord:
    ...
    def from_dict(cls, d):
        return cls(
            ...
            image_uri =d.get("image_uri", d.get("image", "crosvm")),
...
class CrosvmSandboxService(SandboxService):
    _PORT_BASE  = 18000
    _PORT_RANGE = 1000
    ...
    def _alloc_port(self):
        if not self._port_pool:
            raise HTTPException(
                ...
                detail={"code": "CROSVM::NO_PORTS", "message": "No free ports available."},
            )
    ...
    def create_sandbox(self, request: CreateSandboxRequest) -> CreateSandboxResponse:
        ...
        # Running directly inside Debian VM — no proot/crosvm wrapper needed
        full_cmd = [
            "bash", "-c",
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail={"code": "CROSVM::LAUNCH_FAILED", "message": str(exc)},
            ) from exc

        # BUG 4 FIX: ImageSpec.uri not .name
        record = _SandboxRecord(
            sandbox_id=sandbox_id,
            image_uri =request.image.uri if request.image else "crosvm",
    ...
    def get_sandbox(self, sandbox_id: str) -> Sandbox:
        ...
        if not record:
            raise HTTPException(
                ...
                detail={"code": "CROSVM::NOT_FOUND", "message": f"Sandbox {sandbox_id} not found."},
            )
        ...
        if record.pid:
            try:
                ...
                raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail={"code": "CROSVM::PAUSE_FAILED", "message": str(exc)}) from exc
    ...
    def resume_sandbox(self, sandbox_id: str) -> None:
        ...
        if record.pid:
            try:
                ...
                raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail={"code": "CROSVM::RESUME_FAILED", "message": str(exc)}) from exc
    ...
    def renew_expiration(self, sandbox_id: str,
        ...
        if new_expires.replace(tzinfo=timezone.utc) <= now:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                detail={"code": "CROSVM::INVALID_EXPIRY",
