...
for package_arch in "${TERMUX_ARCHITECTURES[@]}"; do
	...
	# DevBox: crosvm is provided by Android AOSP at /apex/com.android.virt/bin/crosvm
	# (available on Android 13+ devices). Create a wrapper script instead of bundling binary.
	if [[ "$package_arch" == "aarch64" || "$package_arch" == "x86_64" ]]; then
		echo "[*] Adding DevBox crosvm wrapper + socat..."
		mkdir -p "${BOOTSTRAP_ROOTFS}/${TERMUX_PREFIX}/bin"
		# Wrapper: use AOSP crosvm with --disable-sandbox (no minijail in Termux context)
		cat > "${BOOTSTRAP_ROOTFS}/${TERMUX_PREFIX}/bin/crosvm" <<'WRAPPER'
#!/data/data/com.termux/files/usr/bin/bash
AOSP_CROSVM=/apex/com.android.virt/bin/crosvm
if [ ! -x "$AOSP_CROSVM" ]; then
    echo "[DevBox] crosvm not found at $AOSP_CROSVM" >&2
    echo "[DevBox] Requires Android 13+ with virtualization support." >&2
    exit 1
fi
exec "$AOSP_CROSVM" --disable-sandbox "$@"
WRAPPER
		chmod 755 "${BOOTSTRAP_ROOTFS}/${TERMUX_PREFIX}/bin/crosvm"
		pull_package socat  # needed for devbox-shell vsock proxy
	fi

	# Add termux bootstrap second stage files
	add_termux_bootstrap_second_stage_files "$package_arch"

	# Add DevBox second stage script
	echo "[*] Adding DevBox second stage script..."
	mkdir -p "${BOOTSTRAP_ROOTFS}/${TERMUX_PREFIX}/etc/termux/bootstrap"
	cp "$TERMUX_SCRIPTDIR/scripts/bootstrap/devbox-second-stage.sh" \
		"${BOOTSTRAP_ROOTFS}/${TERMUX_PREFIX}/etc/termux/bootstrap/devbox-second-stage.sh"
	chmod 700 "${BOOTSTRAP_ROOTFS}/${TERMUX_PREFIX}/etc/termux/bootstrap/devbox-second-stage.sh"

	# DevBox shared dir — mounted into VM at /mnt/devbox via virtio-fs.
	# opensandbox + mobile-agent live here (Termux side), NOT baked into debian.img.
	# Update them = just update bootstrap zip, no need to rebuild the OS image.
	if [[ "$package_arch" == "aarch64" || "$package_arch" == "x86_64" ]]; then
		if [[ -d "$TERMUX_SCRIPTDIR/devbox/opensandbox" ]]; then
			echo "[*] Adding OpenSandbox to shared devbox dir..."
			mkdir -p "${BOOTSTRAP_ROOTFS}/${TERMUX_PREFIX}/share/devbox/opensandbox"
			cp -r "$TERMUX_SCRIPTDIR/devbox/opensandbox/." \
				"${BOOTSTRAP_ROOTFS}/${TERMUX_PREFIX}/share/devbox/opensandbox/"
		fi
		if [[ -d "$TERMUX_SCRIPTDIR/devbox/mobile-agent" ]]; then
			echo "[*] Adding Mobile Agent to shared devbox dir..."
			mkdir -p "${BOOTSTRAP_ROOTFS}/${TERMUX_PREFIX}/share/devbox/mobile-agent"
			cp -r "$TERMUX_SCRIPTDIR/devbox/mobile-agent/." \
				"${BOOTSTRAP_ROOTFS}/${TERMUX_PREFIX}/share/devbox/mobile-agent/"
		fi
	fi
